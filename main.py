...
Join @xbots1 For Updates
[Source Code](https://github.com/r0ld3x/terabox-downloader-bot) """
    check_if = await is_user_on_chat(bot, "@xbots1", m.peer_id)
    if not check_if:
        return await m.reply("Please join @XBOTS1 then send me the link again.")
    check_if = await is_user_on_chat(bot, "@XBOTS1", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @XBOTS1 then send me the link again."
        )
...
async def handle_message(m: Message):
    ...
    if is_spam and m.sender_id not in [7125905015]:
    ...
    if int(data["sizebytes"]) > 524288000 and m.sender_id not in [7125905015]:
        return await hm.edit(
